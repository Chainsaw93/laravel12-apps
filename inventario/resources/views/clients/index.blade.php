    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <a href="{{ route('clients.create') }}" class="mb-4 inline-block bg-blue-500 text-white px-4 py-2 rounded">{{ __('New') }}</a>
                <table class="w-full text-left border">
                    <thead>
                        <tr>
                            <th class="border px-2 py-1">{{ __('Name') }}</th>
                            <th class="border px-2 py-1">{{ __('Email') }}</th>
                            <th class="border px-2 py-1">{{ __('Phone') }}</th>
                    ...
                    <tbody>
                        @foreach($clients as $client)
                            <tr>
                                <td class="border px-2 py-1">{{ $client->name }}</td>
